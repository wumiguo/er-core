...
class SchemaBasedSimJoinECFlowTest extends AnyFlatSpec with SparkEnvSetup {
  ...
  it should "doJoin weighted against 1 attr pair" in {
    ...
    val res = SchemaBasedSimJoinECFlow.doJoin(flowOptions, attrPairArray, true, weights)
    ...
    val res2 = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, true, weights)
    ...
    val res3 = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, true, weights)
  ...
  it should "doJoin unweighted" in {
    ...
    var attrPairArray = ArrayBuffer[(RDD[(Int, String)], RDD[(Int, String)])]()
    attrPairArray :+= (spark.sparkContext.makeRDD(Seq((1, "AE0024"))), spark.sparkContext.makeRDD(Seq((5, "GAE0024"))))
    val res = SchemaBasedSimJoinECFlow.doJoin(flowOptions, attrPairArray, weighted, weights)
    assertResult(true)(res.isEmpty())
    val flowOptions2 = FlowOptions.getOptions(Array("optionSize=3", "option0=algorithm:EDJoin", "option1=threshold:1", "option2=q:2"))
    val res2 = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, weighted, weights)
    assertResult(false)(res2.isEmpty())
    assertResult(List((1, 5, 1.0)))(res2.collect.toList)
    attrPairArray = ArrayBuffer[(RDD[(Int, String)], RDD[(Int, String)])]()
    attrPairArray :+= (spark.sparkContext.makeRDD(Seq((1, "AE0024"))), spark.sparkContext.makeRDD(Seq((5, "AE0024"))))
    val res3 = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, weighted, weights)
    ...
    val res4 = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, weighted, weights).collect.toList
  ...
  it should "doJoin weighted simple" in {
    ...
    val res0 = SchemaBasedSimJoinECFlow.doJoin(flowOptions0, attrPairArray, true, weights).collect.toList
    assertResult(List())(res0)
    var res = SchemaBasedSimJoinECFlow.doJoin(flowOptions, attrPairArray, true, weights).collect.toList
    ...
    res = SchemaBasedSimJoinECFlow.doJoin(flowOptions2, attrPairArray, true, weights).collect.toList
